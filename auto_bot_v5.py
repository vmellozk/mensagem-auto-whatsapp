...
for linha in pag_plan_cliente.iter_rows(min_row=2):
    ...
    mensagem_personalizada = f'Olá {nome}, sua internet vence no dia {vencimento.strftime('%d/%m/%Y')}! Para evitar o 
    bloqueio da linha, efetue o pagamento pelo link: https://www.pagarinternet.com.br'
    ...
    #"Exception" para capturar todas as exceções que ocorrerem. // "as e" atribui o objeto de exceção à variável "e"
        ...
        try:
            #De novo, fechando a página inicial
            pyautogui.hotkey('ctrl', 'w')
            ...
            webbrowser.open(link_direto_wpp)
            time.sleep(8)
            #De novo, localizando o ícone a ser clicado
            clicar_icone_enviar = pyautogui.locateCenterOnScreen('icone_enviar.png')
            time.sleep(1)
            #De novo, passando a coordernada da imagem utilizada
            pyautogui.click(clicar_icone_enviar[0], clicar_icone_enviar[1])
            time.sleep(8)
            #De novo, combinando tecla para fechar a aba
            pyautogui.hotkey('ctrl', 'w')
